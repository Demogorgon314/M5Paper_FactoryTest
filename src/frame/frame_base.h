...
class Frame_Base {
public:
    explicit Frame_Base(bool _has_title = true);
    ...
    int isRun() const { return _is_run; }

    void SetFrameID(uint32_t id) { _frame_id = id; }

    uint32_t GetFrameID() const { return _frame_id; }
...
protected:
    ...
    String _frame_name;
    int _is_run = 1;
    M5EPD_Canvas *_canvas_title = nullptr;
    EPDGUI_Button *_key_exit = nullptr;
    uint32_t _frame_id;
};
