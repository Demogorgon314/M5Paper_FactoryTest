...
void Frame_WifiScan::Connect() {
    ...
    WiFi.disconnect();
    if (_connect_password.indexOf('%') != -1) {
        int index = _connect_password.indexOf('%');
        String identity = _connect_password.substring(0, index);
        String password = _connect_password.substring(index + 1);
        log_d("IDENTITY = [%s]\n", identity.c_str());
        log_d("PASSWORD = [%s]\n", password.c_str());
        esp_wifi_sta_wpa2_ent_set_identity((uint8_t *) identity.c_str(), identity.length());
        esp_wifi_sta_wpa2_ent_set_username((uint8_t *) identity.c_str(), identity.length()); //provide username --> identity and username is same
        esp_wifi_sta_wpa2_ent_set_password((uint8_t *) password.c_str(), password.length()); //provide password
        esp_wpa2_config_t config = WPA2_CONFIG_INIT_DEFAULT(); //set config settings to default
        esp_wifi_sta_wpa2_ent_enable(&config); //set config settings to enable function
        WiFi.begin(_connect_ssid.c_str()); //connect to wifi
    }else{
